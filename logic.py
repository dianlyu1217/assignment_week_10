...
class Game:
	def __init__(self, single: bool):
		...
		if self.turn is self.x:
			self.firstMover = self.x.get_name()
		else:
			self.firstMover = self.y.get_name()
		self.show_board()
	...
	def show_board(self):
		for row in range(len(self.board)):
			...
			for col in range(len(self.board[row])):
				if self.firstMovePosition == '' and self.board[row][col] != ' ':
					if (row == 0 and col == 0) or (row == 0 and col == 2) or (row == 2 and col == 0) or (row == 2 and col == 2):
						self.firstMovePosition = 'Corner'
					elif row == 1 and col == 1:
						self.firstMovePosition = 'Center'
					else:
						self.firstMovePosition = 'Edge'
		print('---current board---')
	...
	def collect_data(self, winner: str):
		...
		try:
			...
			with open(path, 'w') as f:
				w = csv.writer(f)
				h = ["Winner", "FirstMover", "FirstMovePosition"]
				w.writerow(h)
		
		with open(path, 'a') as f:
			w = csv.writer(f)
			h = [winner, self.firstMover, self.firstMovePosition]
			w.writerow(h)
